...
指定间隔学习率衰减：
    milestones = [50, 125, 160] 里面定义的是要调整学习率的 轮数
    gamma:      学习率衰减系数，即：lr新 = lr旧 * gamma
...
# 1.定义函数，演示：等间隔学习率衰减
def dm01():
    # 1.定义变量，记录初始的学习率，训练的轮数，每轮训练的批次数
    ...
    # epoch_list = [0,2,3,...,199]
    # lr_list = [0.1, 0.1, 0.1, 0.05...]
    lr_list, epoch_list = [], []

    # 6.循环遍历训练轮数，进行具体的训练
    for epoch in range(epochs):     # epoch的值:0 ~ 199
