...
梯度下降相关介绍：
    ...
    存在的问题：
        ...
        3.可能会遇到 局部最小值
    解决思路：
        从上述的学习率 或者 梯度入手，进行优化，于是有了：动量法Momentum，自适应学习率 AdaGrad，RMSProp,综合衡量：Adam
    ...
    动量法Momentum:
        ...
        解释：
            St:     本次的指数移动加权平均结果
            B:      调节权重系数，越大，数据越平缓，历史指数移动加权平均 比重越大，本次梯度权重越小
            St-1:   历史指数移动加权平均结果
            Gt:     本次计算出的梯度（不考虑历史梯度）
        加入动量法后的 梯度更新公式：
            W新 = W旧 - 学习率 - St
    ...
    自适应学习率：RMSProp -> 可以看做是 对AdaGrad做的优化，加入 调和权重系数
        公式：
            ...
            梯度下降公式：
                W新 = W旧 - 调整后的学习率 * Gt
        优点：
            RMSProp通过引入 衰减系数B，控制历史梯度 对 历史梯度信息获取的多少

    自适应距估计:Adam(Adaptive Moment Estimation)
        思路:
            即优化学习率，又优化梯度
        公式:
            一阶矩:算均值
                Mt = B1 * Mt-1 + (1 - B1) * Gt          充当：梯度
                St = B2 * St-1 + (1 - B2) * Gt * Gt     充当：学习率
            二阶矩:梯度的方差
                Mt^ = Mt / (1 - B1^t)
                St^ = St / (1 - B2^t)
            权重更新公式:
...
# 4. 定义函数，演示：梯度下降优化方法 -> 自适应距估计(Adam)
def dm04_adam():
    # 1.初始化权重参数
    w = torch.tensor([1.0], requires_grad=True, dtype=torch.float32)
