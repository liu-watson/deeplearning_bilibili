...
def train(x, y, coef):
    ...
    for epoch in range(epochs):         # epoch的值:0,1,2...99
        total_loss = 0.0  # 每轮总损失
        total_sample = 0  # 每轮批次数量

        # 6.3 每轮是分批次训练的，所以从数据加载器中 获取批次数据
        ...
        # 6.8 把本轮的（平均）损失值，添加大列表中
        loss_list.append(total_loss/total_sample)
        print(f"轮数：{epoch + 1}, 本轮平均损失值：{total_loss/total_sample}")

    # 7.打印（最终的）训练结果
    print(f"{epochs}轮的平均损失分别是为：{loss_list}")
