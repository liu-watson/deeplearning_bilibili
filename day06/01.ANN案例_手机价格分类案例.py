...
背景：
    基于手机的20列特征 -> 预测手机的价格区间（4个区间），可以用机器学习做，也可以用深度学习做（推荐）
...
class PhonePriceModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # 正常写法，但是不需要，后续用多分类交叉熵损失函数 CrossEntropyLoss()替代
...
def train(train_dataset, input_dim, output_dim):
    ...
    for epoch in range(epochs):
        ...
        for x, y in train_loader:
            ...
            # 5.2.8 累计损失值
            total_loss += loss.item()   # 把本轮的每批次（16条）的平均损失累计起来。第1批次的平均损失 + 第2批次的平均损失 + ...
            batch_num += 1
...
def evaluate(test_dataset, input_dim, output_dim):
    ...
    # 3.创建测试集的数据加载器对象
    # 参1:测试集对象（400条），参2:每批次的数据条数，参3:是否打乱数据（训练集：打乱，测试集：不打乱）
    ...
    for x, y in test_loader:
        ...
        # 5.3 根据加权求和，得到类别，用argmax()获取最大值对应的下表，就是类别
    ...
    print(f"准确率(Accuracy)：{(correct / len(test_dataset)) * 100:.2f}%")
...
if __name__ == '__main__':
    ...
    # 计算模型参数
    # 参1:模型对象， 参2:输入数的形状(批次大小，输入特征数)，每批16条，每条20列特征
