...
卷积神经网络介绍：
    ...
    组成：
        卷积层（Convolutional):
            用于提取图像的局部特征，结合 卷积核（每个卷积核 = 1个神经元）实现，处理后的结果叫：特征图
        池化层（Pooling)：
            用于 降维，降采样，获取：特征图(Feature Map)
        全连接层（Fully Connected, fc, linear)
            用于 预测结果，并输出结果的
        特征图计算方式：
            N = (W - F + 2 * P)/S + 1
            W:输入图像的大小
...
def dm01():
    ...
    # 4.因为这里只有1张图，所以我们给它再增加1个维度，从CHW -> (1, C, H, W), 1张3通道的640*640像素的图
