...
# t5 = torch.cat([t1, t2], -1) # 效果同上
# print(f"t5:{t5}, shape:{t5.shape}")

# t6 = torch.cat([t1, t2], 2)     # 报错
# print(f"t6:{t6}, shape:{t6.shape}")

print("-" * 30)

# 思路2:stack()拼接张量，可以是新维度，但是无论新旧维度，所有维度都必须保持一致
# t7 = torch.stack([t1, t2], 0)   # (2,3) + (2,3) = (2, 2, 3)
# print(f"t7:{t7}, shape:{t7.shape}")
#
# t8 = torch.stack([t1, t2], 1)   # (2,3) + (2,3) = (2, 2, 3)
# print(f"t8:{t8}, shape:{t8.shape}")

t9 = torch.stack([t1, t2], 2)   # (2,3) + (2,3) = (2, 2, 3)
print(f"t9:{t9}, shape:{t9.shape}")

# t10 = torch.stack([t1, t2], 3)   # (2,3) + (2,3) = (2, 3, ?, 3)
