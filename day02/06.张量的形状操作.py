...
def dm01():
    ...
    # t3 = t1.reshape(2, 5)   # 报错，转之前共计2*3=6个元素，转之后共计2*5=10个元素，不一致
    # print(f"t3:{t3}")
...
def dm04():
    # 思路：演示view()无法改变 不连续的张量的形状，可以通过 is_contiguous()判断张量是否连续，也可以通过contiguous()把不连续的张量->连续张量
    # 1.定义张量
    t1 = torch.randint(1, 10, size=(2, 3))
    print(f"t1:{t1}, shape:{t1.shape}")
    ...
    # 5.尝试把 t3 张量从(3,2)，通过view()转成(2,3)
    # t4 = t3.view(2, 3)                  # t3不连续，所以view()无法改变形状，报错。
    # print(f"t4:{t4}, shape:{t4.shape}")

    # 6.可以通过contiguous()函数，把 t3张量->连续张量->然后就能通过view修改形状了
