...
循环网络层作用：
    基于上一次的隐藏状态 + 本次的输入 -> 本次的隐藏状态，本次的输出

    公式：
        本次的隐藏状态 = tanh(上次的隐藏状态加权求和 + 本次的输入加权求和)
        本次的输出 = 本次的隐藏状态加权求和，在词汇表中所有词的概率，选概率最大的那个词 作为最终预测结果

简单总结下RNN:
    词嵌入层：
        将词（词的索引）转换为词向量表示
    RNN层（循环网络层）：
        逐步处理词向量，生成每个时间步的隐藏状态
    全连接层（输出映射）：
        通过线性变换将隐藏状态映射到输出，通常是1个词汇表中词的概
"""
...
# 参3:隐藏状态向量维度，和上述的hidden_size一样，256维
h0 = torch.randn(size = (1, 32, 256))

# 4.调用RNN处理，获取到当前时刻的预测值和当前的隐藏状态
# 参1:x，本次的输入，参2:h0，上一时刻的隐藏状态

# 返回值1 output:每个时间步的输出，包含了所有时间步的隐藏状态 每看1帧画面，大脑能记住的剧情 -> 共5帧，32个人，256维
# 返回值2 h1:最后1个时间步的隐藏状态，看完最后1帧画面后，大脑里的最新剧情记忆 -> 只关注最后一帧
output, h1 = rnn(x, h0)
