...
def build_vocab():
    ...
    # 7.返回结果：唯一词列表（5703个词），词表{'想要':0, '有':1, ...}, (去重后)词的数量，歌词文本用词表索引表示
...
class LyricsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 3.3 输入值，从文档中取出start - end 的索引的词 -> 作为x
...
def train():
    ...
    lyrics_data_loader = DataLoader(lyrics, batch_size=5, shuffle=True)
    ...
    # 7.2 具体的每轮训练动作
    for epoch in range(epochs):     # epoch:0, 1, 2, 3, ..., 9, 分别表示：第1轮，第2轮，第3轮，第4轮，第5轮，第6轮，第7轮，第8轮，第9轮，第10轮
        # 7.3 定义变量记录：本轮开始训练时间，迭代(批次)次数，训练总损失
        start, iter_num , total_loss = time.time(), 0, 0.0

        # 7.4 具体的本轮各批次训练动作
        # 遍历数据集，后台会调用 LyricsDataset # 的 __getitem__() 方法，获取到每个样本的数据和标签，
        for x, y in lyrics_data_loader:
            # 7.5 获取隐藏层初始值
...
if __name__ == '__main__':
    ...
    # print(f"每个词的索引：{word_to_index}")          # {'想要': 0, '有': 1, '直升机': 2, '\n': 3, '和': 4, '你': 5,...]
    # print(f"文档中每个词对应的索引：{corpus_idx}")    # [0, 1, 2, 3, 40, 0, 4, 5, 6, 7, 8, 3, 40, 0, 4, ...]
