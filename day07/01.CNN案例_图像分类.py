...
回顾：深度学习项目的步骤
    1.准备数据集
        这里我们用的时候 计算机视觉模块torchvision自带的CIFAR10数据集，包含6W张（32，32，3）的图片，5W张训练集，1W张测试集，10个分类，每个分类6K张图片
        你需要单独安装一下torchvision包，即：pip install torchvision
    2.搭建（卷积）神经网络
    3.模型训练
    4.模型测试
...
def create_dataset():
    # 1.获取训练集
    # 参1:数据集路径，参数2：是否是训练集，参数3：数据预处理 -> 张量数据，参数4：是否联网下载（直接用我给的，不用下）
...
def evaluate(test_dataset):
    ...
    for x, y in dataloader:
        ...
        # 5.3 因为训练的时候用了CrossEntropyLoss，所以搭建神经网络时没有加softmax()激活函数，这里要用argmax()来模拟
